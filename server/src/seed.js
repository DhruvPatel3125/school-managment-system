...
const runSeed = async () => {
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      logger.info('🔄 Checking database collections for seed data...');

      // A. Seed Demo School Tenants
      const tenantCount = await Tenant.countDocuments();
      if (tenantCount === 0) {
        logger.info('🌱 Seeding demo school tenants...');
        await Tenant.create([
          {
      ...
      if (permissionCount === 0) {
        logger.info('🌱 Seeding system permission modules...');
        ...
        logger.info('✅ Permission-Role mappings established.');
      }

      // D. Seed Default User Accounts
      const userCount = await User.countDocuments();
      if (userCount === 0) {
        logger.info('🌱 Seeding default login accounts...');
      ...
      if (classCount === 0) {
        logger.info('🌱 Seeding default school classes...');
        const schoolATenant = await Tenant.findOne({ subdomain: 'schoola' });
        const schoolBTenant = await Tenant.findOne({ subdomain: 'schoolb' });
        ...
        if (schoolATenant && schoolBTenant) {
          ...
          if (studentCount === 0) {
            logger.info('🌱 Seeding default students...');
            ...
            logger.info('✅ Default students seeded.');
          }
        }
      }
      ...
      if (staffCount === 0) {
        logger.info('🌱 Seeding default staff directory...');
        const schoolATenant = await Tenant.findOne({ subdomain: 'schoola' });
        const schoolBTenant = await Tenant.findOne({ subdomain: 'schoolb' });
        ...
        if (schoolATenant && schoolBTenant) {
          ...
          logger.info('✅ Default staff directory seeded.');
        }
      }
      ...
      if (attendanceCount === 0 || assignmentCount === 0 || feeCount === 0) {
        logger.info('🌱 Seeding demo attendance, assignments, and fees...');
        ...
        if (aarav && dia && schoolATenant) {
          ...
          if (attendanceCount === 0) {
            ...
            logger.info('✅ Demo attendance logs seeded.');
          }
          ...
          if (assignmentCount === 0) {
            ...
            logger.info('✅ Demo assignments seeded.');
          }
